...
- Fetches `origin/<PR base branch>` before resolving that merge-base and fails the step closed if the fetch or merge-base cannot be resolved, rather than falling back to an empty-tree or push-delta tip that regenerates an unbounded first-turn review
- Names that integration branch in the review prompt and forbids the agent from replacing the named merge-base with `origin/main` or the forge default
...
- Bounds each agent turn independently with [`review_agent_timeout`](/no-mistakes/reference/global-config/#review_agent_timeout) for wall clock and [`agent_stall_timeout`](/no-mistakes/reference/global-config/#agent_stall_timeout) for progress: the optional fixer and its fresh, session-free rereviewer each receive the full wall-clock limit, as do every later fixer and rereviewer; a progressless turn cancels on the stall bound even though Review installs the wall-clock deadline first, and reaching either hard limit fails the step with measured last-activity, no-output, or no-progress evidence rather than leaving the run active indefinitely
