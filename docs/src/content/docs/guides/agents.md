...
Transient API and network failures, stochastic prose turn endings, transient tool-call or permission validation errors, and final-text parse failures whose remedy is a fresh turn (provider protocol residue after a complete object, or a split answer the parser could not fuse) are retried up to three times with exponential backoff. A structured invocation that completes with no assistant text at all is retried the same way by the adapters that parse structured output from the final text - the shared `finalizeTextResult` path used by Codex, Copilot, ACP targets through `acpx`, Antigravity, OpenCode, Pi, and Rovo Dev: it is a harness outcome rather than a malformed answer, since a JSON-stream harness legitimately ends a turn on a thinking-only or toolCall-only assistant message, and it is deliberately not reported as a structured-output rejection - no answer existed to reformat - so it never counts against a step's own structured-output retry bound, and a review step only spends those attempts on answers that genuinely need steering back to the schema. An adapter with a native structured-output channel has no final text to parse, so an empty turn reaches it as a missing structured output instead: Claude retries that shape at the adapter layer and still reports the rejection to the step, and Grok does not retry it at all, so for both the step's own attempts are still spent. A schema-less invocation that returns no text keeps its plain, non-retryable failure: no structured answer was requested, so that verdict does not apply to it. Provider quota and free-usage-limit errors are terminal, even when a backend marks them retryable. Retry messages are recorded as lifecycle activity for native subprocess agents, falling back to the streaming text path for direct callers that do not supply `OnLifecycle`.

A schema violation names the values it rejected. The review step quotes the validation error back to the agent when it reruns a review whose final JSON did not match the schema, so an enum message lists the allowed values (`must match one of the allowed values "source-or-external", "pipeline-owned-delivery"`) instead of leaving the agent to guess which of two similarly named enums applied - the failure that had a gate agent repeat the same rejected answer.
