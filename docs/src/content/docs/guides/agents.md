...
Transient API and network failures, stochastic prose turn endings, transient tool-call or permission validation errors, and final-text parse failures whose remedy is a fresh turn (provider protocol residue after a complete object, or a split answer the parser could not fuse) are retried up to three times with exponential backoff. A turn that completes with no assistant text at all is retried the same way: it is a harness outcome rather than a malformed answer, since a JSON-stream harness legitimately ends a turn on a thinking-only or toolCall-only assistant message, and it is deliberately not reported as a structured-output rejection - no answer existed to reformat. It also never counts against a step's own structured-output retry bound, so a review step only spends those attempts on answers that genuinely need steering back to the schema. Provider quota and free-usage-limit errors are terminal, even when a backend marks them retryable. Retry messages are recorded as lifecycle activity for native subprocess agents, falling back to the streaming text path for direct callers that do not supply `OnLifecycle`.
